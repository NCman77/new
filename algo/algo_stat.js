/**
 * algo_stat.js V3.3 - The Statistical Analysis Algorithm (統計學派 - 強化健全性版)
 * 
 * ==========================================
 * 版本資訊
 * ==========================================
 * V3.3 (2026-01-02) - 確定性增強（P0-致命Bug修復）
 * - ✅ [T17-P0] 自動生成確定性seed（Top模式無seed時）
 * - ✅ 基於資料hash生成seed（期數+最新一期號碼）
 * - ✅ metadata新增 autoGeneratedSeed, seed 欄位
 * 
...
function stat_generateTicketLotto(ctx) {
    ...
    const meta = {
        ...
        mode,
        seed: rng._seed,
        ...(ctx.autoGeneratedSeed ? { autoGeneratedSeed: true } : {}),
        shape,
...
export function algoStat(params = {}) {
    ...
    // ===== T17: 自動生成確定性seed（V3.3新增）=====
    // 當 seed===null 且 mode==='top' 時，自動生成基於資料的seed
    let effectiveSeed = seed;
    let autoGeneratedSeed = false;

    if (seed === null && safeMode === 'top') {
        // 基於資料特徵生成seed：期數 + 最新一期號碼
        const dataLen = normalizedData.length;
        const firstDraw = normalizedData[0];
        let hashInput = dataLen;

        if (firstDraw && firstDraw.mainNumbers && Array.isArray(firstDraw.mainNumbers)) {
            hashInput = firstDraw.mainNumbers.reduce((acc, n) => (acc * 49 + n) | 0, dataLen);
        } else if (firstDraw && firstDraw.digits && Array.isArray(firstDraw.digits)) {
            hashInput = firstDraw.digits.reduce((acc, d) => (acc * 10 + d) | 0, dataLen);
        }

        effectiveSeed = hashInput | 0;  // 確保為整數
    ...
    if (packMode) {
        ...
        const packMeta = {
            ...
            seed: rng._seed,
            ...(autoGeneratedSeed ? { autoGeneratedSeed: true } : {}),
